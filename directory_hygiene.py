...
def check_and_update_rescaled_paths(nwb_paths: tuple[pathlib.Path, ...]) -> None:
    rescaled_paths = (path for path in nwb_paths if 'rescaled' in str(path))
    if not rescaled_paths:
        return
    
    non_rescaled_paths = (path for path in nwb_paths if 'rescaled' not in str(path))
    for path in non_rescaled_paths:
        path.unlink()
    
    for path in rescaled_paths:
        str_path = str(path)
        new_path = str_path.replace('rescaled', '')
...
def remove_non_nwb_files(nwb_session_path: pathlib.Path) -> None:
    ...
    non_nwb_paths = (path for path in all_paths if path not in nwb_paths)

...
def save_stimulus_table_json(session_parameters:dict, non_doc=False, is_dynamic_gating:bool=False, is_vbn_opto:bool=False):
    session_parameters, input_json = get_ecephys_stimulus_table_json(session_parameters, non_doc=non_doc, is_dynamic_gating=is_dynamic_gating,
...
def save_nwb_json(session_parameters: dict, session:np_session.Session, probe_lookup_table_path:pathlib.Path, 
    ...
    if len(probe_lookup_table) > 0 and session.id in probe_lookup_table['session'].values:
    ...
    unit_start = 0
    for probe in session_parameters['probes']:
        if channel_lookup_table_path.exists():
            channel_lookup_table = pd.read_csv(channel_lookup_table_path)
        else:
            channel_lookup_table = pd.DataFrame()
        
        if units_lookup_table_path.exists():
            unit_lookup_table = pd.read_csv(units_lookup_table_path)
        else:
            unit_lookup_table = pd.DataFrame()
        ...
        session_parameters, input_json, channel_start, unit_start = generate_nwb_input_json(session_parameters, session, isi_areas, structure_tree, 
                                                                 probe_id, channel_lookup_table, channel_lookup_table_path,
                                                                 unit_lookup_table, units_lookup_table_path,
...
def generate_sdk_modules(session_str: str, is_dynamic_gating=False, is_vbn_opto=False, non_doc=False, scale=1):
    ...
    if is_vbn_opto:
        probe_lookup_table_path = pathlib.Path(vbn_lookup_base_path, 'probes_lookup_table.csv')
        channel_lookup_table_path = pathlib.Path(vbn_lookup_base_path, 'channels_lookup_table.csv')
